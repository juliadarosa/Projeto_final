...
Como deve ser salvo arquivo . Configurações da máquina virtual Box
...
`vagrant halt`  **Vagrant:**  Virtual Box

**halt:**  comando de parar Vagrant

# Acessando a máquina e verificação

comandos de para acessar o ambiente de máquina virtual e verifação de versionamento.  `vagrant ssh`
...
Como navegar palas pastas compartilhadas no Vangrant Comandos:
...
Segue comando de instalação:  `rvm install 2.3`
...
-   criar pasta em Ruby:  `rails new nome_pasta`
...
# Adicionando banco de dados
 Especificação do banco na utilização da Aplicação

`rails _5.2_ new -d postgresql`

-   criar pasta em Ruby:   Neste caso a utilização do banco foi escolhido postgres.

## Utilizando Scaffold
 Scaffold seria para criação de um crud  para uma tabela em especifica do banco de dados.
 Segue comando : 

`rails generate scaffold < model> < campo.tipo <campo:tipo>...`

Segue comando para criação do banco e passando seus campos e tipo.

-   Executar comandos SQL em diretório :  `rails dbconsole`

## comandos para gerenciamento no Rails

Segue comandos de gerenciamento neste exemplo seria para gerenciamento do banco de dados : 

-   **Criação de tabela** : `rails db:create`

-   **Deletar tabela** : `rails db:drop`

-   **Permite ações no banco sem utilizar SQL:** : `rails db:migrate`

-   **Reverter ação no banco:** : `rails db:rolllback`

Neste documento foi possível verificar alguns principais comandos utilizados . 
